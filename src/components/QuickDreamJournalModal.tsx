...
export const QuickDreamJournalModal: React.FC<QuickDreamJournalModalProps> = ({
    ...
    try {
      ...
      setCameraError(
        err.name === 'NotAllowedError'
          ? 'Camera access was denied. Please allow permissions or upload an image.'
          : 'Could not access camera. You can upload an image from your device.'
      );
      setIsCameraActive(false);
    }
  };
  ...
  const handleSave = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      soundSynthesizer.playTapChime();
      showToast('✨ Dream Journal entry captured!', 'success');
      resetForm();
      onClose();
    } catch (err) {
      console.error(err);
  ...
  return (
    <Modal
      ...
      title="Quick Dream Journal Capture"
      subtitle="Capture immediate inspirations, mindset breakthroughs, or future reflections instantly from anywhere in the app."
      maxWidth="lg"
    >
      <div onKeyDown={handleKeyDown} className="space-y-4">
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <Field
            ...
            required
            helper="What decision, vision moment, or realization happened?"
          >
            <Input
              id="quick-journal-title"
              ref={titleInputRef}
              ...
              placeholder="e.g. Breakthrough clarity on high-leverage quarterly milestone"
            />
          </Field>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field id="quick-journal-mood" label="Current State of Mind">
              <Select
            ...
            <Field
              ...
              label="Linked Vision Target (Optional)"
              helper="Connect to a specific future milestone."
            >
              <Select
                id="quick-journal-dream"
                value={linkedDreamId}
          ...
          <Field
            ...
            required
            helper="Describe the insight, mindset shift, or execution victory."
          >
            <Textarea
              id="quick-journal-content"
              value={content}
              onChange={(e) => setContent(e.target.value)}
              placeholder="Jot down the specific circumstances, thoughts, or next decision to solidify..."
              rows={4}
            />
          </Field>
            ...
            {isCameraActive ? (
              <div className="w-full h-56 rounded-[var(--radius-md)] overflow-hidden relative bg-black border-2 border-[var(--color-sage)] shadow-inner">
                ...
                <div className="absolute top-2.5 left-2.5 bg-black/70 backdrop-blur-xs px-2 py-0.5 rounded text-[10px] text-white flex items-center gap-1.5">
                  ...
                  Live Viewfinder ({cameraFacing === 'user' ? 'Front' : 'Back'})
                </div>
              ...
              >
                ...
                <div className="flex flex-col items-center justify-center gap-2">
                  ...
                  <span className="text-[11px] text-[var(--fg-subtle)]">
                    Drag and drop an image or use your device camera
                  </span>
                </div>
              </div>
            )}
