...
export const Logo: React.FC<LogoProps> = ({ className = 'w-6 h-6', title }) => (
  <svg viewBox="280 130 700 640" className={className} aria-hidden={title ? undefined : true} role={title ? 'img' : undefined}>
...
export const LogoLockup: React.FC<LogoProps> = ({ className = 'w-12 h-18', title = 'ODA' }) => (
  <svg viewBox="280 130 700 1000" className={className} role="img" aria-label={title}>
    <image href={brandImage} width="1254" height="1254" />
  </svg>
);
